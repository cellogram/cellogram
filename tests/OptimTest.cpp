...
const auto func_quadratic_invert = [](auto x, auto y, auto z) -> double {
    if ((x-14.25)*(x-14.25) + (y-14.75)*(y-14.75) <= 4*4) {
        return 1;
    } else {
        return 0;
...
TEST_CASE("optim_ideal_invert", "[OptimTest]") {

	image_t image; // 30 * 30 * 30
    GenImage(image, func_quadratic_invert);
    double thres = QuantileImage(image, 1.0);
    NormalizeImage(image, thres);
    ...
    optim::Optim_FixDepth(optimPara, bsplineSolver, CI, CO, EO, IterO, true);  // inverted color
    ...
    REQUIRE(good >= 40);
    REQUIRE(bad <= 1);  //  TODO: same as the non-invert case
    REQUIRE(failure <= 9);
}
