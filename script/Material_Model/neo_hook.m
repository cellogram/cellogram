...
% This script is used to fit a neo-Hookean incompressible material model to test
...
% equibiaxial test data to the neo-Hookean model. The parameter "weighting" in the
...
load('StrainStressTest_10_to_9.mat')
...
% 
% u_ft = fittype('a*(x^2 - 1/ x)');
% e_ft = fittype('a*(x^2 - 1/ x^4)');
...
max_ind = round(numel(fx_e)*weighting);
fun = @(a)(sum((fx_u-a*(x_u.^2-1./x_u)).^2) +...
    sum((fx_e(1:max_ind) - a*( x_e(1:max_ind).^2 - 1./ x_e(1:max_ind).^4 )).^2));
options = optimset('Display','iter','TolFun',1e-12,'TolX',1e-12);
mu_2 = fminsearch(fun,1,options);
...
figure
subplot(1,2,1)
plot(u.lambda1,u.cauchy_stress,'LineWidth',3,'Color',[41, 128, 185]./255)
ylabel("Cauchy Stress [MPa]")
xlabel("Stretch [-]")
title("Uniaxial")
hold on
plot(u.lambda1,mu_2*(u.lambda1.^2 - 1./ u.lambda1),'LineWidth',1.5,'Color',[39, 174, 96]./255)
legend({'Data','Fit'},'Location','northwest')

subplot(1,2,2)
plot(e.lambda1,e.cauchy_stress,'LineWidth',3,'Color',[41, 128, 185]./255)
ylabel("Cauchy Stress [MPa]")
xlabel("Stretch [-]")
title("Equibiaxial")
hold on
plot(e.lambda1,mu_2*(e.lambda1.^2 - 1./ e.lambda1.^4),'LineWidth',2,'Color',[39, 174, 96]./255)

%% convert cauchy to nominal
u.fit_cauchy_stress = mu_2*(u.lambda1.^2 - 1./ u.lambda1);
e.fit_cauchy_stress = mu_2*(e.lambda1.^2 - 1./ e.lambda1.^4);
